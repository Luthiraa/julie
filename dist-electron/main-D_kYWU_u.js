...
import require$$0$2, { deprecate, inspect } from "util";
...
class Blob2 {
  ...
  slice(start, end, contentType) {
    return new Blob2(sliceBlob(__classPrivateFieldGet$4(this, _Blob_parts, "f"), this.size, start, end), {
      type: contentType
    });
  }
  async text() {
...
function requireAgent() {
  ...
  const debug2 = require$$0$2.debuglog("agentkeepalive");
  const {
    INIT_SOCKET,
    CURRENT_ID,
    CREATE_ID,
...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-CAtQ2qPA.js");
...
class APIClient {
  constructor({
    ...
    __classPrivateFieldSet(this, _APIClient_baseURLOverridden, baseURLOverridden);
  ...
  async buildRequest(inputOptions, { retryCount = 0 } = {}) {
    ...
    const { method, path: path2, query, defaultBaseURL, headers = {} } = options;
  ...
  buildURL(path2, query, defaultBaseURL) {
    const baseURL = !__classPrivateFieldGet(this, _APIClient_baseURLOverridden) && defaultBaseURL || this.baseURL;
...
function updateGroqClient(newKey) {
...
-   **Always** use the tool when a command is needed. Do not just suggest the command properly.
-   **Output**: If you use a tool, your response will be processed by the system.
`;
const TERMINAL_TOOL_DEF = {
  type: "function",
  function: {
    name: "execute_terminal_command",
    description: "Execute a terminal command on the user's machine (Windows PowerShell). Use this to read files, list directories, create files, or check system status.",
    parameters: {
      type: "object",
      properties: {
        command: {
          type: "string",
          description: "The command line to execute (e.g., 'dir', 'type file.txt', 'node -v')."
        }
      },
      required: ["command"]
    }
...
ipcMain.handle("set-api-key", (_2, key) => {
  console.log("Received request to update API Key via IPC.");
  updateGroqClient(key);
  return true;
});
ipcMain.handle("transcribe-audio", async (_2, arrayBuffer) => {
  try {
...
ipcMain.handle("run-command", async (_2, command) => {
...
Stderr: ${stderr}`);
      ...
      const output = stdout.trim() || stderr.trim() || "Command executed successfully (no output).";
      resolve(output);
    });
  });
});
async function askGroqWithFallback(messages, model = "llama-3.3-70b-versatile", retries = 1, tools = null) {
  try {
    const params = {
      messages,
      model
...
ipcMain.handle("ask-groq", async (_2, args) => {
  if (!currentApiKey) {
...
function createWindow() {
  ...
  ipcMain.handle("set-content-protection", (_2, protect) => {
    if (win) {
      win.setContentProtection(protect);
    }
  });
  ipcMain.handle("resize-window", (_2, { width: width2, height: height2 }) => {
    if (win) {
      const { workArea: workArea2 } = screen.getPrimaryDisplay();
      const x22 = Math.round(workArea2.x + (workArea2.width - width2) / 2);
      const y22 = workArea2.y + 20;
      win.setBounds({ x: x22, y: y22, width: width2, height: height2 }, true);
    }
  });
...
app.whenReady().then(() => {
  ...
  globalShortcut.register("CommandOrControl+Up", () => {
    if (win) {
      const { x: x2, y: y2, width, height } = win.getBounds();
      win.setBounds({ x: x2, y: y2 - MOVE_STEP, width, height });
    }
  });
  ...
  globalShortcut.register("CommandOrControl+Left", () => {
    if (win) {
      const { x: x2, y: y2, width, height } = win.getBounds();
      win.setBounds({ x: x2 - MOVE_STEP, y: y2, width, height });
    }
  });
