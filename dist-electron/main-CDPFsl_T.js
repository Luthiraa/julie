...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-Btau1dnm.js");
...
You are Cluely, a live-meeting co-pilot.
Goal: Help the user based on the conversation transcript.

Priorities:
1. Answer direct questions concisely.
2. Define technical terms/proper nouns in the last 15 words.
3. Suggest follow-ups if no question.
...
function createWindow() {
  ...
  ipcMain.handle("resize-window", (_2, { width, height }) => {
    if (win) {
