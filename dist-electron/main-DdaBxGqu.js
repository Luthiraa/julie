...
import { ipcMain, app, BrowserWindow, globalShortcut, screen } from "electron";
...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-CC0AQPoL.js");
...
ipcMain.handle("ask-groq", async (_2, messages) => {
  ...
  return await askGroqWithFallback(fullMessages);
});
