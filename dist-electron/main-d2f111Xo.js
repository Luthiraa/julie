...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-CGog-5Cn.js");
...
- **ALWAYS** use newlines between sections and bullet points.
...
- There is no clear question, inquiry, or request for information at the end of the transcript.
- There is no company name, technical term, product name, or domain-specific proper noun within the final 10–15 words.
- There is no clear or visible problem or action item present.
</when_to_enter_passive_mode>
...
ipcMain.handle("ask-groq", async (_2, prompt) => {
  try {
